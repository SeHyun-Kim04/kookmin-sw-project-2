...
void loop()
{
  ...
  if(_DIST_MIN <= dist_raw && dist_raw <= _DIST_MAX){ // Put range Filter code here (_DIST_MIN ~ _DIST_MAX)
    // and turn on LED if the distance is in the range 
  
    dist_ema = _EMA_ALPHA * dist_raw + ((1-_EMA_ALPHA) * dist_ema);
        
    //duty = map(dist_ema, _DIST_MIN, _DIST_MAX, _DUTY_MIN, _DUTY_MAX);
    duty = self_made_map(dist_ema, _DIST_MIN, _DIST_MAX, _DUTY_MIN, _DUTY_MAX);// Put map() equivalent code here
...
float self_made_map(float x, float in_min, float in_max, float out_min, float out_max) {
